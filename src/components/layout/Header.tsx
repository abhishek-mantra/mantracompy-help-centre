...
import { buildGmailComposeUrl, SUPPORT_TEMPLATES } from "../../utils/supportLinks";

export function Header() {
  const submitRequestUrl = buildGmailComposeUrl(SUPPORT_TEMPLATES.generalRequest);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-18">
          ...
          <div className="flex items-center gap-4 sm:gap-6 text-xs sm:text-sm font-semibold">
            ...
            {/* Direct Gmail autofill redirect — no stranded mailto and no modal */}
            <a
              href={submitRequestUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-600 hover:text-[#043570] transition-colors hidden sm:inline-flex items-center gap-1 font-medium"
