...
export function TableOfContents({ items }: TableOfContentsProps) {
  ...
  return (
    <nav
      ...
      className="space-y-3 p-4 bg-slate-50/80 border border-slate-200/90 rounded-2xl text-xs shadow-2xs"
    >
      <div className="flex items-center gap-2 font-bold uppercase tracking-wider text-slate-500 text-[11px] pb-2 border-b border-slate-200/80">
        <List className="size-3.5 text-[#2196F3]" />
      ...
      <ul className="space-y-1.5 max-h-[calc(100vh-220px)] overflow-y-auto pr-1">
        {items.map((item) => {
          ...
          return (
            <li
              key={item.id}
              className={item.level === 3 ? "pl-3 text-[11px]" : ""}
            >
              <a
                href={`#${item.id}`}
                onClick={(e) => {
                ...
                className={`group flex items-start gap-1.5 py-1 px-2 rounded-lg transition-all ${
                  isActive
                    ? "font-bold text-[#2196F3] bg-blue-50 border-l-2 border-[#2196F3]"
